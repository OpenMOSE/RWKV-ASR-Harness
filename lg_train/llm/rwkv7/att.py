...
class RWKV7_TMIX(nn.Module):
    ...
    def forward(
        ...
        #     x , _ = RUN_RWKV7_STATE(r,k,v,w,-kk, kk*a,past_state)
        # else:
        # The wkv7 kernel (CUDA & fla/triton) operates strictly in bf16; cast
        # inputs so the model works under both pure-bf16 and bf16-mixed (autocast
        # with fp32 master weights) precision.
...
#     @torch.compile
#     def forward(self, x, v_first, attention_mask=None, state=None):
#         B, T, C = x.size()
#         H = self.n_head
