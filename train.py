...
parser.add_argument("--wandb_mode", default="online", type=str)  # online / offline / disabled
...
parser.add_argument("--save_per_steps", default=0, type=int)  # also save every N steps (0 = off)
parser.add_argument("--keep_last_ckpt", default=5, type=int)  # keep only the N most recent rwkv-step-*.pth (0 = keep all)
parser.add_argument("--inspect_grad", default=0, type=int)  # grad-norm inspection: print every N steps (0=off). works under DeepSpeed
parser.add_argument("--inspect_layer", default=0, type=int)  # which block index gets a per-parameter grad-norm breakdown
parser.add_argument("--spike_thresh", default=0, type=float)  # if grad_norm > this, dump the offending batch's per-sample stats (0=off)
parser.add_argument("--debug_data", default=0, type=int)  # 1 = print which dataset samples are used each step
...
#LoRA (independent for time-mix / ffn; 0 = off)
parser.add_argument("--lora_tmix", default=0, type=int)   # rank for time-mix receptance/key/value/output
parser.add_argument("--lora_ffn", default=0, type=int)    # rank for ffn key/value
parser.add_argument("--lora_alpha", default=0, type=float)  # 0 -> alpha=rank (scaling 1.0)
