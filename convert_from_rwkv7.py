...
def convert(
    ...
    proj_weights = {}  # 用于存储 proj 权重

    for name, weight in weights.items():
        if name.startswith("proj.") or name.startswith("encoder."):
            proj_weights[name] = weight.clone()  # 单独保存
            continue
    ...
    # assert False
    # print("Saved proj weights to output/proj_weights.safetensors")        
    
    renamed_weights = {}
    for name, tensor in weights.items():
       
        if name.startswith("llm."):
            new_name = name.replace("llm.", "", 1)
        else:
            new_name = name
        renamed_weights[new_name] = tensor
    ...
    for name in weights:
        if 'proj.' in name or 'encoder.' in name:
            continue
        print(name)    
    ...
    for k, v in proj_weights.items():
        new_k = "model." + k
        merged[new_k] = v.clone()
    for k, v in llm_weights.items():
        new_k =  k.replace('model.','model.llm.')
        merged[new_k] = v.clone()
    # 保存合并后的 safetensor
    os.makedirs(os.path.dirname(output+'/model.safetensors') or ".", exist_ok=True)
    save_file(merged, output+'/model.safetensors')
    print(f"Saved merged safetensor to {output+'/model.safetensors'}")
